...
## 2024-05-17 - Optimize /cost-centers aggregation in legacy backend
**Learning:** When grouping by an alias in SQLAlchemy/Postgres, if the alias matches an existing table column name, Postgres will group by the original column's raw values. Furthermore, `func.coalesce` only handles `NULL` values and will not replace empty strings unless explicitly converted to `NULL` via `func.nullif`.
**Action:** Pass the exact SQLAlchemy expression to `.group_by()` instead of the string label, and use `func.coalesce(func.nullif(Column, ''), 'default')` for conditional fallback logic on empty strings.
