...
## 2026-08-03 - [Optimize Batch Operations with Promise.all]
**Learning:** Sequential await calls in a loop (e.g. `for (const id of ids) { await this.approveManual(...) }`) cause performance bottlenecks by making operations wait for the previous one to finish, even when they are independent.
**Action:** When working on batch operations where each iteration is independent, use `Promise.all` with `.map` to execute them concurrently, significantly reducing overall execution time.
