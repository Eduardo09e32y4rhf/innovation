...
## 2027-01-01 - Batch independent Prisma queries
**Learning:** Sequential, independent database queries inside schedule service endpoints block execution unnecessarily. Waiting for each query before starting the next adds latency equivalent to the sum of all query times.
**Action:** Group independent queries using \`Promise.all()\` to run them concurrently, effectively reducing the latency to just the time of the longest single query.
