...
## 2024-05-24 - Consolidate Prisma Aggregations
**Learning:** Executing multiple independent `.aggregate()` calls on the same table within a `Promise.all` can be inefficient due to multiple database roundtrips.
**Action:** Use `.groupBy()` to consolidate these queries into a single call, reducing connection overhead. Safely handle missing groups using optional chaining (`?.`) and nullish coalescing (`??`), as `.groupBy()` omits groups with zero records.
