...
## 2026-09-11 - [Avoid N+1 queries in cron tasks / triggers]
**Learning:** Cron jobs or trigger functions (like `triggerPeriodicAso` in AsoService) that loop over records and check database existence inside the loop (e.g., `findFirst` followed by `create`) can cause N+1 performance bottlenecks.
**Action:** When finding loops running database queries, pre-fetch all necessary related data using `findMany` and `in: [...]`, index the data in-memory (e.g., Map, Set), and replace individual creates with bulk `createMany()`. Ensure type safety and that duplicates are correctly handled within the batch itself.
