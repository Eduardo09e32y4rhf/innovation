...
## 2024-05-18 - Optimized N+1 query in ASO generation
**Learning:** In ASO onboarding background cron job, findFirst was used inside a for loop resulting in N+1 queries.
**Action:** Replaced iterative findFirst queries with a bulk findMany using the 'in' operator on employeeIds, loaded the most recent record into a Map, and grouped records to perform a single createMany operation for both ASO records and notifications.
