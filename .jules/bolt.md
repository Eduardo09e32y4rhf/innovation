...
## 2026-08-17 - Dynamic Map updates during N+1 resolution
**Learning:** When resolving N+1 loops where the iterations create database records using data that will be queried in later iterations (e.g., verifying if a record for `employeeId` already exists before creating), a simple pre-loop `findMany` into a `Map` is insufficient. The `Map` state will be stale for subsequent loop iterations sharing the same identifiers, potentially causing duplicate creations.
**Action:** When replacing `findFirst` in a loop with an in-memory `Map`, dynamically update the `Map` in memory (e.g., `existingMap.set(employeeId, newRecord.createdAt)`) immediately after creating the new record inside the loop to ensure correct state across iterations.
