...
## 2024-05-23 - Batch DB Operations in NestJS background cron jobs
**Learning:** Background cron jobs and initial module load cycles that loop over records sequentially issuing .create() or .update() calls incur severe N+1 transaction overhead in Prisma, heavily penalizing execution times.
**Action:** When looping over multiple records to perform the same query on different row IDs, always refactor using .createMany() or .updateMany() combined with 'in' filters, and wrap multiple updateMany statements in a single $transaction. Add explanatory comments to indicate the optimization.
