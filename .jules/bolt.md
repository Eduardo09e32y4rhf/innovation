...
## 2026-05-05 - Eager Loading with existing Joins
**Learning:** When a query already uses `.join(Model)` for filtering purposes, using `.options(joinedload(Model))` creates redundant joins (one inner join and one left outer join).
**Action:** Use `.options(contains_eager(Model))` instead of `joinedload` when the model is already explicitly joined in the query to map the explicitly joined columns to the relationship.
