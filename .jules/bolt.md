...
## 2024-05-14 - Dashboard Kanban N+1 Queries
**Learning:** In SQLAlchemy, explicitly using `.join(Model)` to filter records does not automatically make the joined records available on the relationship attribute (e.g., `app.job`). Accessing the relationship later still triggers a lazy load query.
**Action:** Always combine `.join(Model)` with `.options(contains_eager(Parent.model))` when you need to access that relationship data in Python after fetching, to prevent silent N+1 performance degradations.
