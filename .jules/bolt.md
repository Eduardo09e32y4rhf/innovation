...
## 2024-05-25 - Database Aggregation for Cost Centers
**Learning:** In the `get_cost_centers` endpoint, fetching all transaction objects for a given company into memory to group them iteratively causes massive memory overhead (O(N) mapping from SQL rows to ORM objects, plus list structures). This anti-pattern becomes a severe bottleneck for large user datasets. Using `func.sum()` combined with a `group_by()` using `func.coalesce` correctly offloads this grouping operation and total calculation directly to the database.
**Action:** Always prefer `group_by()` in SQL queries instead of retrieving all objects into Python to do category aggregations manually. When grouping fields that might be NULL or empty strings, `func.coalesce(func.nullif(field, ''), 'Default')` must be passed both into the SELECT expression and directly into `.group_by()` to prevent SQL logic/aggregation mismatches.
