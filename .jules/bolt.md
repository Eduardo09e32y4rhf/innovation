...
## 2025-03-15 - [Backend Kanban Dashboard N+1 Queries]
**Learning:** `get_kanban_board` in `dashboard.py` loops over applications fetching `app.candidate` and `app.job` dynamically triggering N+1 queries.
**Action:** Use `.options(joinedload(Application.candidate), contains_eager(Application.job))` alongside existing joins to eager load relationships efficiently.
