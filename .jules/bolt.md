...
## 2025-01-30 - Fix N+1 queries in ASO generation
**Learning:** Database performance degrades severely when making iterative Prisma ORM queries inside a loop such as `for (const record of expired)` and conditionally checking and creating new records based on that.
**Action:** Extract database dependencies by pre-fetching bulk data using `{ in: [...] }` filtering, then regrouping results in memory (e.g. Map objects by employee ID) to iterate locally without emitting further SQL queries. Update the Map progressively in the loop so if the same ID appears again, it registers the newly created item and skips creation if one was just inserted.
