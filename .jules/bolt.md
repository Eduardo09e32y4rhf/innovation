...
## 2026-09-02 - [Batching Prisma creates in OnModuleInit]
**Learning:** Initializing default configuration data or seeding databases using Prisma in NestJS `OnModuleInit` lifecycle hooks with iterative `create` queries causes N+1 query patterns that delay module startup.
**Action:** Combine static seeding data in memory and use a single `createMany` operation to prevent these N+1 query patterns during module startup.
