...
## 2026-06-14 - Concurrent Database Inserts using Promise.all in TypeScript
**Learning:** Sequential await loops for inserting arrays of entities (like `candidateSeeds`) cause unnecessary I/O wait time bottlenecks. Node.js backend processes often iterate over lists using `for (const x of arr) { await db.insert(x); }` which resolves records synchronously rather than dispatching them to the database pool simultaneously.
**Action:** When inserting or processing mutually independent records in arrays, replace sequential `for...of` loops with `await Promise.all(arr.map(async (x) => { ... }))` to parallelize database I/O operations and dramatically reduce the execution time of the block.
