...
## 2024-05-20 - CI Pipeline Performance Optimization
**Learning:** Running `black . --check` globally in a CI pipeline can cause unnecessary failures if legacy files are modified or if the codebase is large, leading to slow CI times. Furthermore, when `working-directory` is set in GitHub Actions, `git diff --name-only` returns paths relative to the repo root, which breaks `xargs black` when executed inside a subdirectory.
**Action:** Use `git diff --relative --name-only` to constrain diff paths to the current working directory, making the conditional linting check resilient to subdirectory executions. Always use `fetch-depth: 0` in checkout when running diffs in CI to ensure the base refs exist.
