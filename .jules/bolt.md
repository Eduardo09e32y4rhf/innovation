...
## 2024-06-08 - Prisma Aggregate Consolidation
**Learning:** In the NestJS backend, executing multiple independent `.aggregate()` queries on the same table (e.g., separate aggregations for REVENUE and EXPENSE) within `Promise.all` causes unnecessary database roundtrips and connection overhead.
**Action:** Always consolidate multiple independent `.aggregate()` calls into a single `.groupBy()` query when operating on the same table and filtering conditions. Safely extract values using `Array.prototype.find()` with optional chaining and nullish coalescing to handle omitted zero-record groups.
