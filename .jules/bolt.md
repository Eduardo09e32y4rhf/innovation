...
## $(date +%Y-%m-%d) - CI/CD GitHub Actions Workflow paths
**Learning:** The monorepo has its backend and frontend apps located inside the `apps/` directory (e.g., `apps/frontend`, `apps/backend`). The `.github/workflows/ci-cd.yml` configuration had incorrect default `working-directory` paths pointing to the root-level `./frontend` and `./backend`, causing pipeline resolution errors. Additionally, `package-lock.json` caching and `npm install` failures occur when the repository relies on `pnpm`.
**Action:** Always ensure GitHub Actions `working-directory` maps properly to the subdirectories of `apps/`. Use `pnpm` (`npm install -g pnpm && pnpm install` and `pnpm run build`) for frontend builds and update cache keys to `pnpm-lock.yaml`.
