...
## 2024-05-18 - [Optimize seed routines during NestJS initialization]
**Learning:** During NestJS `OnModuleInit` hooks, running repetitive Prisma `create` queries inside an iterative `for...of` loop creates unnecessary N+1 overhead during application startup, which delays boot times as the system seeds initialization data.
**Action:** Consolidate iterative seeding structures into a single array and execute a single Prisma `createMany` operation instead to dramatically optimize application boot time.
