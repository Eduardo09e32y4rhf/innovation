...
## 2025-03-08 - Prisma Aggregate Consolidation
**Learning:** Multiple `.aggregate()` calls on the same table in Prisma create separate database queries, even when run inside a `Promise.all()`.
**Action:** Consolidate independent `.aggregate()` queries into a single `.groupBy()` query with conditional aggregation in memory to reduce database roundtrips.
