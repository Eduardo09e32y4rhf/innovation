...
## 2024-06-03 - Consolidating Prisma aggregate calls
**Learning:** Calling `.aggregate()` multiple times in a `Promise.all` for different subsets of the same table results in multiple separate database queries.
**Action:** Consolidate multiple independent `.aggregate()` queries into a single `.groupBy()` query to minimize database connection overhead and roundtrips, extracting the grouped data on the application layer.
