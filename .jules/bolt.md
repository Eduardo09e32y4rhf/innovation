...
## 2024-05-15 - Prisma Aggregate Consolidation
**Learning:** Executing multiple independent `.aggregate()` calls on the same table within a `Promise.all` adds unnecessary connection overhead and database roundtrips.
**Action:** Consolidate multiple `.aggregate()` calls into a single `.groupBy()` query, using optional chaining and nullish coalescing to safely handle missing groups in the result array, as `groupBy` omits groups with zero records.
