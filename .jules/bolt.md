...
## 2024-06-25 - [Optimize N+1 Queries in Time Closing Generation]
**Learning:** Database operations executed inside loops over large arrays of entities (such as iterating over employees to delete and insert records one by one) create severe N+1 performance bottlenecks.
**Action:** When updating or inserting records for multiple entities in a single operation, construct an array of the payloads in memory and execute the queries iteratively using bulk methods like `deleteMany` and `createMany` outside the loop.
