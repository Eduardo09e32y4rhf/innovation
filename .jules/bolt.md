...
## 2024-05-18 - Fix N+1 queries in loop replacing create with createMany
**Learning:** When resolving N+1 Prisma query problems by refactoring iterative `.create()` calls into bulk `.createMany()` operations, remember that `.createMany()` does not return the created records.
**Action:** If the newly created records (and their relations) are required for return values, execute a subsequent `.findMany()` query using an `in` filter on the identifiers (e.g., `employeeId: { in: ids }`) to fetch them.
