...
## 2024-06-04 - Design System Component ARIA Propagation
**Learning:** Generic design system components (like icon-only table action buttons) often drop critical accessibility attributes because their specific context isn't known to the component itself.
**Action:** Always design reusable generic interactive components to expose and propagate `aria-label` and `title` props so consumer components can inject localized context.
