...
## 2024-09-02 - ARIA labels for Chat Interface Elements
**Learning:** Custom chat interfaces (like WhatsApp clones) frequently use icon-only buttons for actions like sending, attaching, and returning to the chat list. These are critical interaction points but often lack `aria-label` attributes, making them inaccessible to screen reader users.
**Action:** When working on complex interaction components with floating or icon-only buttons, always ensure that each button has a descriptive Portuguese `aria-label` attribute (e.g., "Enviar mensagem", "Anexar arquivo") to maintain accessibility.
