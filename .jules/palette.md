...
## 2025-05-31 - Propagating Accessibility Props in Reusable Components
**Learning:** Generic reusable icon buttons (like `TableActionButton`) in the enterprise design system cannot hardcode `aria-label` since their context changes. Hardcoding limits accessibility for specific consumer components.
**Action:** Always ensure generic interactive components expose and propagate accessibility props (like `aria-label` and `title`) to allow consumers to inject localized, context-specific descriptions.
