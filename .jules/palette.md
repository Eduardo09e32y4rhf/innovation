...
## 2025-03-05 - Dynamic ARIA Labels in State Toggles
**Learning:** When an interactive element like a dropdown button displays dynamic text (e.g., the currently selected language), setting a static `aria-label` completely overrides the inner text for screen readers. This prevents users from perceiving the current state of the control.
**Action:** Always incorporate the dynamic value into the `aria-label` (e.g., `aria-label={\`Select Language, currently ${currentLanguage?.name}\`}`) to preserve context while adding accessibility improvements.
