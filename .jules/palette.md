...
## 2024-05-24 - Accessibility for Icon-Only Buttons in Banners
**Learning:** Icon-only buttons used for dismissing banners require not only `aria-label` for screen readers but also explicit focus states (`focus-visible:outline-none focus-visible:ring-2`) so keyboard users know they have focus, and a `title` attribute so mouse users get a tooltip explaining the action.
**Action:** Always ensure any icon-only button added to a notification or banner component includes `aria-label`, `title`, and `focus-visible` Tailwind classes.
