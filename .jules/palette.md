...
## 2024-05-18 - Added accessibility to AnnouncementBanner close button
**Learning:** Icon-only close buttons (like `<X />`) lack inherent accessible names, making them invisible to screen readers, and often lack explicit keyboard focus styles for navigation.
**Action:** Always add `aria-label`, `title`, and explicit focus styles (e.g., `focus-visible:ring`) to icon-only buttons to ensure they are accessible for keyboard navigation and screen readers.
