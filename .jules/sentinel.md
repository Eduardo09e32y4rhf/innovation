...
## 2026-06-13 - Missing Authorization Checks in Controllers
**Vulnerability:** Mutation endpoints in UsersController and CompaniesController allowed any authenticated user to create, update, or delete records, including changing user roles, leading to privilege escalation.
**Learning:** By default, controllers protected only with `JwtAuthGuard` do not enforce role-based access control, even if a `RolesGuard` exists in the codebase.
**Prevention:** Always apply `RolesGuard` at the controller level and explicitly decorate sensitive mutation methods with `@Roles('ADMIN')` while leaving read methods accessible to standard users.
