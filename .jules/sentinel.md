...
## 2025-03-04 - [HIGH] Information Disclosure via Exception Stack Traces in AI Endpoints
**Vulnerability:** Similar to previous findings, the AI endpoints (`legacy/IA/gemini/api/endpoints/ai.py`) were returning the raw output of Python exceptions (`str(e)`) directly to users via HTTP 500 response bodies or direct response payloads when interacting with language models.
**Learning:** Returning `str(e)` directly in HTTP responses can leak sensitive internal details about API keys, quota limits, and backend processing steps. The principle of failing securely requires these to be caught internally.
**Prevention:** Catch exceptions, log `str(e)` securely on the backend using Python's `logging` library, and return a sanitized, generic error message (e.g. "Erro interno ao processar a requisição") to the client. Always implement standard python logging instead of raw prints for these tracking events.
