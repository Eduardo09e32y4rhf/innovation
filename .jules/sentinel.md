...
## 2024-06-02 - Privilege Escalation in User Management
**Vulnerability:** The UsersController lacked role-based authorization (RolesGuard), allowing any authenticated user to create, modify, or delete other users and potentially grant themselves admin privileges.
**Learning:** Applying JwtAuthGuard alone only verifies authentication in NestJS. Endpoints managing sensitive resources must explicitly apply RolesGuard and @Roles() decorators.
**Prevention:** Always verify that both JwtAuthGuard and RolesGuard are applied together with appropriate @Roles() constraints on controllers handling user management.
