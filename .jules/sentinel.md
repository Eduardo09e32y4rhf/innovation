...
## 2024-05-19 - Hardcoded Sandbox API Key in Codebase
**Vulnerability:** A hardcoded ASAAS sandbox API key was found as a fallback value for `os.getenv` in `AsaasService`.
**Learning:** Even sandbox or test keys should never be hardcoded in the repository as they can lead to accidental leaks or be promoted to production if the code structure is copied. Defaulting to an empty string or failing securely is preferred.
**Prevention:** Always require API keys to be explicitly injected via the environment. Use configuration validation to fail fast if required secrets are missing, rather than falling back to hardcoded defaults.
