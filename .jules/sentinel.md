...
## 2025-06-06 - [HIGH] Missing Authorization Checks on User Endpoints
**Vulnerability:** The `UsersController` lacked proper role-based access control, allowing any authenticated user to potentially list, view, modify, or delete other users because it only used `JwtAuthGuard` and was missing `RolesGuard`.
**Learning:** Authentication (verifying identity) does not automatically imply Authorization (verifying permissions). Endpoints handling user management must explicitly check privileges using both `JwtAuthGuard` and `RolesGuard` to prevent privilege escalation.
**Prevention:** Apply `RolesGuard` and the corresponding `@Roles(...)` decorator on all controllers or endpoints that deal with user administration.
