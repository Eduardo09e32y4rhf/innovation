...
## 2026-05-05 - DoS vulnerability on resource-intensive health endpoints
**Vulnerability:** Missing authentication on `/health/deep` endpoint allows unauthenticated DoS attacks by triggering actual email sends.
**Learning:** Resource-intensive endpoints, even if intended for internal health checks, must be secured if accessible externally.
**Prevention:** Always secure resource-intensive health checks with `Depends(get_current_user)` to prevent unauthenticated access.
