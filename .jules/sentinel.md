...
## 2025-03-04 - [HIGH] Information Disclosure via Exception Stack Traces in Finance Endpoints
**Vulnerability:** The `/finance/transactions` POST and GET endpoints were returning the raw output of Python exceptions (`str(e)`) directly to users via HTTP 500 response bodies.
**Learning:** Returning `str(e)` directly in HTTP responses can leak sensitive internal details, database structure (SQLAlchemy errors), or logic to malicious actors. This violates the principle of failing securely and "Never expose raw exception strings (`str(e)`) in HTTP responses to external clients."
**Prevention:** Catch exceptions, log `str(e)` securely on the backend using Python`s `logging` library, and return a sanitized, generic error message (e.g. "Erro interno ao criar transação") to the client.
