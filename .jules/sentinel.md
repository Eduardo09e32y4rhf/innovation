...
## 2025-03-04 - [HIGH] Missing Generic Error Messages in AI service and endpoints
**Vulnerability:** Endpoints handling AI and third party logic in `backend/src/services/innovation_service.py` and `backend/src/api/v1/endpoints/ai.py` were exposing internal Python exception structures (`str(e)`) directly in HTTPException messages.
**Learning:** Returning `str(e)` during an exception in a service or API router can leak internal API paths, logic structure, external dependency failures, and potentially sensitive prompt metadata to unauthorized users. It is imperative to always sanitize error responses before yielding to the user.
**Prevention:** Use standard exception logging `logger.error(str(e))` to retain debug capability and replace the details passed to `raise HTTPException` with concise, hardcoded generic strings without variable interpolations or exception stacks.