...
## 2025-03-04 - [HIGH] Authentication Bypass and Timing Attack in Asaas Webhook
**Vulnerability:** The `/api/payments/webhook` endpoint for Asaas payments was vulnerable to an authentication bypass because it only checked the `asaas-access-token` header if the `ASAAS_WEBHOOK_TOKEN` environment variable was set. If missing, it allowed unauthenticated requests. Additionally, it used standard string inequality (`!=`), opening the endpoint to timing attacks.
**Learning:** Security configurations must fail closed. If an expected webhook token isn't configured, the endpoint must reject requests (HTTP 500) rather than processing them. Using `!=` for token comparison is a common mistake that allows timing attacks; secure comparators must always be used.
**Prevention:** Always verify webhook signatures securely using `secrets.compare_digest`. Ensure that missing security configurations result in a strict rejection (fail-closed behavior) rather than bypasses.
## 2025-03-04 - [CRITICAL] Hardcoded API Key in Source Code
**Vulnerability:** A hardcoded API key (`ASAAS_API_KEY`) was present in `backend/src/services/asaas_service.py` as a fallback value for an environment variable.
**Learning:** Even if it's a sandbox or testing key, committing secrets to source control is a critical security violation. It exposes the credentials to anyone with read access to the repository, potentially leading to unauthorized access, resource consumption, or lateral movement depending on the key's privileges.
**Prevention:** Never use hardcoded secrets as fallback values in `os.getenv` or similar configuration mechanisms. Always rely entirely on secure environment variable injection or secret management systems. If a required secret is missing, the application should log a clear warning or fail securely during startup.
