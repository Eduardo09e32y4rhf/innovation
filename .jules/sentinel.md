...
## 2025-03-04 - [HIGH] Python 3 Exception Scoping (NameError) leading to 500s or Information Leaks
**Vulnerability:** The exception block `except Exception as inner_e:` was structured such that `inner_e` was accessed *outside* the `except` block scope (`yield f"data: [ERROR] Falha crítica: {str(inner_e)}"`). In Python 3, exceptions bound using `as` are cleared from scope at the end of the `except` block. This resulted in an unhandled `NameError` causing an unexpected 500 failure rather than securely yielding an error message. The attempt itself also violated the guideline of not exposing internal error strings (`str(inner_e)`).
**Learning:** Python automatically deletes the exception instance name at the end of the `except` clause to avoid reference cycles. Accessing it outside the block fails. If the exception information is needed later, it must be assigned to a different variable before the `except` block ends.
**Prevention:** If an exception object is needed after an `except` block, assign it to a differently named variable inside the block. Never expose the exception string to users; log it internally and return/yield a sanitized error message instead.
