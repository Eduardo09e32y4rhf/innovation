...
## 2025-03-04 - [CRITICAL] Privilege Escalation via Missing RolesGuard
**Vulnerability:** The `/users` endpoints in the NestJS API (`apps/api`) were protected by `JwtAuthGuard` but completely missing the `RolesGuard` and `@Roles` decorators. This allowed any authenticated user (e.g., standard users or managers) to call administrative user management endpoints (like listing, creating, and deleting users) and potentially escalate their privileges.
**Learning:** Adding a generic `JwtAuthGuard` is insufficient for endpoints handling user management or other sensitive administrative operations. The presence of a role system is only effective if explicitly enforced via guards on every relevant controller or route.
**Prevention:** Always apply both `JwtAuthGuard` and `RolesGuard` (along with the appropriate `@Roles` decorator, such as `@Roles('ADMIN')`) to endpoints that handle user management or sensitive operations to protect against privilege escalation.
