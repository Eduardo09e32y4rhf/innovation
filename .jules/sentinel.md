...
## 2025-03-04 - [MEDIUM] Information Disclosure via Exception Stack Traces in finance and ai endpoints
**Vulnerability:** The backend endpoints `finance.py`, `finance_das.py`, and `ai.py` were leaking internal exception strings (`str(e)`) in HTTP 500 response bodies by directly passing them to `HTTPException(500, detail=...)`.
**Learning:** Direct pass-through of `str(e)` to `HTTPException` provides attackers with explicit system internals that can be used to construct secondary attacks, like discovering internal DB constraints or API endpoints.
**Prevention:** Always mask `str(e)` behind generic error messages in 5XX responses (e.g. "Erro interno ao buscar transações."). Only expose errors if they are explicit, user-facing validation errors (like 400 Bad Request / ValueError).
