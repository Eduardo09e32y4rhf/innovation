...
## 2025-03-04 - [Fix Information Leakage in API error responses]
**Vulnerability:** Raw Python exception strings (`str(e)`) and external API error messages were being returned directly to the client via FastAPIs `HTTPException` `detail` parameter.
**Learning:** In multiple endpoints, exceptions were caught but their raw string representations were used as the error message, exposing internal details and upstream service errors.
**Prevention:** Always use the `logging` module to log the raw error and raise an `HTTPException` with a generic, safe string instead of exposing internal errors or upstream messages.
