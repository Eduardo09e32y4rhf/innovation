    ...
    for api_key in active_keys:
        try:
            ...
            try:
                ...
                # Só tentamos outra chave se a geração ainda não tiver começado a yieldar dados
                if not started and ("429" in str(e) or "quota" in str(e).lower()):
                    ai_key_manager.mark_as_exhausted(api_key)
                    continue
...
async def landing_plan(data: LandingPlanRequest):
    ...
    try:
        ...
        raise HTTPException(500, detail="Erro interno ao processar a requisição")
