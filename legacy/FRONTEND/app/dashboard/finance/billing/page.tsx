...
import { CreditCard, History, Download, AlertCircle, ExternalLink, TrendingUp, Zap, Users, DollarSign, ArrowUpRight, CheckCircle } from 'lucide-react';
...
const METRICS = [
  ...
  { label: 'Usuários Ativos', value: '3', change: '0%', icon: <Users className="text-blue-400" size={20} />, color: 'text-blue-400' },
...
const PLANS = [
  { name: 'Starter', price: 'R$ 99', features: ['500 chamadas IA/mês', '1 usuário', 'Suporte por email'], current: false },
  { name: 'Professional', price: 'R$ 149', features: ['1000 chamadas IA/mês', '5 usuários', 'WhatsApp Bot', 'Suporte prioritário'], current: true },
  { name: 'Enterprise', price: 'R$ 499', features: ['Ilimitado', 'Usuários ilimitados', 'IA customizada', 'SLA garantido'], current: false },
];
...
<div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-right-4 duration-700">
        <header className="mb-8">
          ...
          <p className="text-gray-400">Gerencie seu plano, métricas de uso e histórico de pagamentos.</p>
        </header>
        ...
        <div className="flex gap-2 mb-8 bg-white/[0.02] p-1.5 rounded-2xl border border-white/5 w-fit">
          {(['overview', 'invoices', 'plans'] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-5 py-2 rounded-xl text-xs font-bold transition-all capitalize ${tab === t ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
              {t === 'overview' ? 'Visão Geral' : t === 'invoices' ? 'Faturas' : 'Planos'}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-8">
              ...
              {METRICS.map((m, i) => (
                <div key={i} className="glass p-6 rounded-3xl border border-white/5 hover:border-white/10 transition-all">
                  <div className="flex justify-between items-start mb-4">
                    <div className="p-2.5 bg-white/5 rounded-xl">{m.icon}</div>
                    <span className={`text-xs font-bold ${m.color}`}>{m.change}</span>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="glass p-8 rounded-3xl border-purple-500/30 relative overflow-hidden">
                ...
                <p className="text-gray-400 text-sm mb-6">R$ 149,00 / mês · Renova em 01/06</p>
                <div className="space-y-3 mb-8">
                  {['1000 chamadas IA/mês', '5 usuários ativos', 'WhatsApp Bot + Automações'].map((f, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs text-gray-300">
                      <CheckCircle size={14} className="text-purple-400" /> {f}
                    </div>
                  ))}
                </div>
                <div className="mb-4">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="glass p-6 rounded-3xl border border-yellow-500/20 bg-yellow-500/[0.03]">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-yellow-400 mb-1">Próxima Cobrança</p>
                      <p className="text-xs text-yellow-500/70 leading-relaxed">Sua assinatura Professional será renovada automaticamente em <strong>01 de Junho de 2026</strong> por <strong>R$ 149,00</strong>. Mantenha seu cartão atualizado.</p>
                    </div>
                  </div>
                </div>
                <div className="glass p-8 rounded-3xl">
                  <h3 className="font-bold mb-6 text-sm flex items-center gap-2"><TrendingUp size={16} className="text-purple-400" /> �altimas Transações</h3>
        ...
        {tab === 'invoices' && (
          <div className="glass p-8 rounded-3xl">
            <h3 className="text-xl font-bold mb-8 flex items-center gap-2"><History size={20} /> Histórico de Faturas</h3>
                ...
                <tbody className="text-sm">
                  {INVOICES.map((inv) => (
                    <tr key={inv.id} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors">
                      <td className="py-4 font-mono text-[10px] text-gray-500">{inv.id}</td>
                      <td className="py-4 font-medium">{inv.plan}</td>
                      <td className="py-4">{inv.date}</td>
                      <td className="py-4 font-mono font-bold">{inv.amount}</td>
                      <td className="py-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PLANS.map((plan) => (
              <div key={plan.name} className={`glass p-8 rounded-3xl border transition-all ${plan.current ? 'border-purple-500 shadow-lg shadow-purple-500/10' : 'border-white/5 hover:border-white/20'}`}>
                ...
                <p className="text-3xl font-black mb-1">{plan.price}<span className="text-sm text-gray-500 font-normal">/mês</span></p>
                <div className="space-y-3 my-8">
                  {plan.features.map((f, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs text-gray-300">
                      <CheckCircle size={14} className={plan.current ? 'text-purple-400' : 'text-gray-500'} /> {f}
                    </div>
                  ))}
                </div>
