...
async def landing_plan(data: LandingPlanRequest):
    ...
    try:
        ...
        logging.getLogger(__name__).error(f"Erro no simulador: {e}")
...
async def generate_video(
    ...
    for api_key in active_keys:
        try:
            ...
            logging.getLogger(__name__).error(f"Erro no Veo: {e}")
            raise HTTPException(
                500, detail="Erro interno ao processar a requisição no Veo"
            )
