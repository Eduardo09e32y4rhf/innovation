...
async def get_kanban_board(
    ...
    # ⚡ Bolt: Eliminate N+1 query problem by eagerly loading candidate and job relationships.
    # Why: Previously, accessing `app.candidate` and `app.job` inside the loop triggered a database query for each application.
    # Impact: Reduces O(N) database queries to O(1), drastically improving load times for the Kanban dashboard with many applications.
    apps = (
        db.query(Application)
