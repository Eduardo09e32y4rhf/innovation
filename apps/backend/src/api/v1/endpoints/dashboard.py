...
async def get_kanban_board(
    ...
    # Why: Previously executed a database query for each application's job and candidate inside the loop
    # Impact: Reduces O(N) database queries to O(1), significantly improving kanban board load time
    apps = (
        db.query(Application)
        .join(Job)
