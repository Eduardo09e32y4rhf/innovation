...
from services.ai_image import ai_image_service
...
def create_job(
    ...
    try:
        ...
        try:
            # Gera banner promocional com IA
            banner_url = ai_image_service.generate_job_banner(job.title)
            # job.banner_url = banner_url # Descomentar quando o campo existir no model
            logger.info(f"Banner gerado: {banner_url}")
...
def get_job_applications(
    ...
    # ⚡ Bolt: Eliminate N+1 query problem by fetching candidates in a single query
