...
export function TableActionButton({
  ...
  return (
    <button
      onClick={onClick}
      className="p-2 bg-white/5 rounded-lg hover:text-purple-400 focus-visible:ring-2 focus-visible:ring-purple-400 outline-none transition-colors"
      type="button"
      aria-label={ariaLabel}
      title={title}
    >
...
export function DownloadPdfButton({
  ...
  return (
    <TableActionButton
      onClick={onClick}
      aria-label={ariaLabel ?? 'Baixar PDF'}
      title={title ?? 'Baixar PDF'}
    >
      <Download size={14} />
    </TableActionButton>
  );
