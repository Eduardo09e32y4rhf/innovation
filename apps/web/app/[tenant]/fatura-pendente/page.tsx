      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-[var(--radius-xl)] border border-white/10 bg-zinc-900/85 p-8 text-center shadow-[var(--shadow-xl)] backdrop-blur-xl">
        ...
        {isAdmin ? (
          <div className="mt-7 space-y-4">
            ...
            {invoiceLink ? (
              <button onClick={() => window.location.assign(invoiceLink)} className="flex h-12 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-[var(--color-brand)] font-black text-white transition hover:bg-[var(--color-brand-600)]">
                <CreditCard size={18} /> Pagar agora no Asaas
              </button>
            ) : (
              <button onClick={() => generateCheckout()} disabled={creating} className="flex h-12 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-[var(--color-brand)] font-black text-white transition hover:bg-[var(--color-brand-600)] disabled:opacity-60">
