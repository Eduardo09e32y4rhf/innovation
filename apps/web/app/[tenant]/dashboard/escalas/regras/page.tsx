          ...
          {loadingRules ? <LoadingState /> : rules.length > 0 ? rules.map((rule: any) => (
            <div key={rule.id} className="card-flat p-4 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                {rule.status === 'ACTIVE' ? (
                  <button className="btn-icon text-gray-600 hover:text-red-500" aria-label="Arquivar" onClick={() => archiveMutation.mutate(rule.id)}>
                    <Archive className="w-4 h-4" />
                  </button>
                ) : (
                  <button className="btn-icon text-green-600" aria-label="Ativar" onClick={() => activateMutation.mutate(rule.id)}>
          ...
          {loadingHolidays ? <LoadingState /> : holidays.length > 0 ? holidays.map((holiday: any) => (
            <div key={holiday.id} className="card-flat p-4 flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                ...
                <button className="btn-icon text-red-600 hover:text-red-800" aria-label="Excluir" onClick={() => handleDeleteHoliday(holiday.id)}>
                  <Archive className="w-4 h-4" />
                </button>
              </div>
            </div>
