          ...
          {loadingRules ? <LoadingState /> : rules.length > 0 ? rules.map((rule: any) => (
            <div key={rule.id} className="card-flat p-4 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="btn-icon" aria-label="Editar regra"><Edit2 className="w-4 h-4" aria-hidden="true" /></button>
                {rule.status === 'ACTIVE' ? (
                  <button className="btn-icon text-gray-600 hover:text-red-500" onClick={() => archiveMutation.mutate(rule.id)} aria-label="Arquivar regra">
                    <Archive className="w-4 h-4" aria-hidden="true" />
                  </button>
                ) : (
                  <button className="btn-icon text-green-600" onClick={() => activateMutation.mutate(rule.id)} aria-label="Ativar regra">
          ...
          {loadingHolidays ? <LoadingState /> : holidays.length > 0 ? holidays.map((holiday: any) => (
            <div key={holiday.id} className="card-flat p-4 flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                ...
                <button className="btn-icon" aria-label="Editar feriado"><Edit2 className="w-4 h-4" aria-hidden="true" /></button>
                <button className="btn-icon text-red-600 hover:text-red-800" onClick={() => handleDeleteHoliday(holiday.id)} aria-label="Excluir feriado">
                  <Archive className="w-4 h-4" aria-hidden="true" />
                </button>
              </div>
            </div>
