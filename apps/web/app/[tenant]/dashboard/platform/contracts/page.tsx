...
export default function ContractsPage() {
  ...
  const [form, setForm] = useState({ companyId: filterCompanyId || '', seatQuantity: 1, agreedAmount: '', startsAt: '', endsAt: '', paymentMethod: 'EXTERNAL', notes: '' });
  
  const load = async () => { 
    const [contracts, companyItems] = await Promise.all([request<any[]>('/manual-contracts'), request<any[]>('/platform/companies')]); 
    setItems(contracts); 
