...
export default function PlatformDashboardPage() {
  ...
  const webhookEvents = useQuery(() => api.platform.finance.webhookEvents({ limit: 80 }), []);

  const refresh = () => { stats.refetch(); summary.refetch(); invoices.refetch(); companies.refetch(); webhookEvents.refetch(); };
  ...
  async function saveCompany(data: any) {
    ...
    try {
      await api.platform.updateCompany(editingCompany.id, data);
      toast.success('Empresa atualizada.');
      setEditingCompany(null);
      companies.refetch();
    } catch (error) {
      setCompanyError(error instanceof ApiError ? error.message : 'Nao foi possivel salvar a empresa.');
    } finally {
  ...
  const failedWebhookEvents = (webhookEvents.data ?? []).filter((event) => event.status === 'FAILED');
  const pendingWebhookEvents = (webhookEvents.data ?? []).filter((event) => event.status === 'PENDING' || event.status === 'PROCESSING');
  const quickActions: QuickAction[] = [
    { label: 'Financeiro', description: 'Cobranças, reembolsos e sincronização', href: `/${tenant}/dashboard/platform/finance`, icon: Banknote, tone: 'bg-sky-50 text-sky-700' },
    { label: 'Contratos', description: 'Criar, editar, excluir e gerar PDF', href: `/${tenant}/dashboard/platform/contracts`, icon: FileSignature, tone: 'bg-violet-50 text-violet-700' },
    { label: 'Configuração', description: 'Empresas, planos e permissões', href: `/${tenant}/dashboard/platform/permissions`, icon: Settings2, tone: 'bg-emerald-50 text-emerald-700' },
  ];
  ...
  return (
    <div className="mx-auto w-full space-y-5 pb-10">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-base font-black text-slate-950">Empresas clientes</h3><p className="mt-1 text-xs text-slate-500">Acoes financeiras e operacionais no mesmo lugar.</p></div><Link href={`/${tenant}/dashboard/platform/companies`} className="inline-flex items-center gap-1 text-xs font-black text-violet-700 hover:underline">Ver todas <ArrowRight size={13} /></Link></div>{companyItems.length === 0 ? <div className="p-8"><EmptyState message="Nenhuma empresa cadastrada." /></div> : <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left text-xs"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3">Empresa</th><th className="px-4 py-3">Plano</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Asaas</th><th className="px-4 py-3 text-right">Acoes</th></tr></thead><tbody className="divide-y divide-slate-100">{companyItems.slice(0, 8).map((company) => { const status = companyStatus(company); const automatic = Boolean(company.asaasSubscriptionId); return <tr key={company.id} className="hover:bg-slate-50/70"><td className="px-4 py-4"><p className="font-black text-slate-950">{company.name}</p><p className="mt-1 text-[10px] text-slate-400">{company.document || 'Sem documento'}</p></td><td className="px-4 py-4 font-bold text-slate-700">{company.plan || 'FREE'}</td><td className="px-4 py-4"><span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${status.className}`}>{status.label}</span></td><td className="px-4 py-4"><span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${automatic ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-500'}`}>{automatic ? 'Automatico ativo' : company.asaasCustomerId ? 'Cliente vinculado' : 'Nao configurado'}</span></td><td className="px-4 py-4"><div className="flex justify-end gap-2"><button type="button" onClick={() => { setCompanyError(null); setEditingCompany(company); }} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-2 text-[11px] font-bold text-slate-700 hover:bg-slate-50"><Settings2 size={13} /> Editar</button><Link href={`/${tenant}/dashboard/platform/${company.id}?tab=general`} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-2 text-[11px] font-bold text-slate-700 hover:bg-slate-50"><ArrowRight size={13} /> Abrir</Link>{isDev && !automatic && company.status === 'ACTIVE' && <button type="button" onClick={() => activateAutomaticBilling(company)} disabled={workingCompany === company.id} className="inline-flex items-center gap-1 rounded-lg bg-violet-600 px-2.5 py-2 text-[11px] font-black text-white hover:bg-violet-700 disabled:opacity-60">{workingCompany === company.id ? <Loader2 size={13} className="animate-spin" /> : <CreditCard size={13} />} Ativar Asaas</button>}</div></td></tr>; })}</tbody></table></div>}</section>

      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 p-4"><div><h3 className="text-base font-black text-slate-950">Saude da integracao Asaas</h3><p className="mt-1 text-xs text-slate-500">Historico de eventos, falhas e retentativas automaticas.</p></div><Link href={`/${tenant}/dashboard/platform/finance`} className="text-xs font-black text-violet-700 hover:underline">Financeiro completo</Link></div><div className="grid gap-3 p-4 sm:grid-cols-3"><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3"><p className="text-[10px] font-black uppercase text-emerald-700">Processados</p><p className="mt-1 text-xl font-black text-emerald-950">{(webhookEvents.data ?? []).filter((event) => event.status === 'PROCESSED').length}</p></div><div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-[10px] font-black uppercase text-amber-700">Em fila</p><p className="mt-1 text-xl font-black text-amber-950">{pendingWebhookEvents.length}</p></div><div className="rounded-xl border border-rose-200 bg-rose-50 p-3"><p className="text-[10px] font-black uppercase text-rose-700">Falhas</p><p className="mt-1 text-xl font-black text-rose-950">{failedWebhookEvents.length}</p></div></div>{webhookEvents.error ? <p className="px-4 pb-4 text-xs font-semibold text-rose-700">Nao foi possivel carregar o historico Asaas.</p> : failedWebhookEvents.length > 0 ? <div className="border-t border-slate-100 divide-y divide-slate-100">{failedWebhookEvents.slice(0, 5).map((event) => <div key={event.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black text-slate-900">{event.eventType} · {event.company?.name || 'Empresa nao identificada'}</p><p className="mt-1 text-[10px] text-rose-600">Tentativas: {event.attempts} · {event.errorMessage || 'Falha sem detalhe'}</p></div>{isDev && <button type="button" onClick={async () => { try { await api.platform.finance.retryWebhookEvent(event.id); toast.success('Evento reenfileirado.'); webhookEvents.refetch(); } catch (error) { toast.error(error instanceof ApiError ? error.message : 'Falha ao reenfileirar evento.'); } }} className="inline-flex items-center justify-center gap-1 rounded-lg bg-rose-600 px-3 py-2 text-[11px] font-black text-white hover:bg-rose-700"><RefreshCw size={13} /> Reprocessar</button>}</div>)}</div> : <p className="border-t border-slate-100 p-4 text-xs text-slate-500">Nenhuma falha recente. Os eventos processados ficam registrados para auditoria.</p>}</section>
      ...
      {editingCompany && <CompanyManageModal company={editingCompany} onClose={() => setEditingCompany(null)} onSave={saveCompany} loading={savingCompany} error={companyError} />}
    </div>
  );
}
