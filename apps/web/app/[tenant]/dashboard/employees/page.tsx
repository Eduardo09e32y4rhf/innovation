...
function downloadEmployeeSheet(employee: Employee, rows: TimeTrack[], month: string, company: Company | null, managerName: string) {
  ...
  const companyData: PdfCompanyInfo | null = company ? {
    name: company.name,
    document: (company as any).cnpj || '',
    address: [(company as any).street, (company as any).streetNumber, (company as any).neighborhood, (company as any).city, (company as any).state, (company as any).cep].filter(Boolean).join(', ')
  } : null;
