...
export default function ChatPage() {
  ...
  const fetchContacts = async () => {
    ...
    setActiveContact((current) =>
      current
        ? nextContacts.find(
            (contact: ChatContact) => contact.id === current.id,
          ) || current
        : nextContacts[0] || null,
    );
  };
  ...
  const fetchMessages = async (jid: string) => {
    ...
    try {
      const res = await fetch(
        `${getApiBaseUrl()}/communication/chats/${encodeURIComponent(jid)}/messages`,
        {
          headers: getAuthHeaders(),
        },
      );
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex justify-between items-center gap-2">
                    <p className="text-xs text-gray-400 truncate">
                      {contact.lastMessage || contact.id.split("@")[0]}
                    </p>
      ...
      <div className="flex-1 flex flex-col bg-black/40 relative">
        {activeContact ? (
            ...
            <div className="h-20 border-b border-white/5 bg-[#0a0a0f]/80 backdrop-blur-md flex items-center justify-between px-6 z-10">
              ...
              <div className="flex items-center gap-4 text-gray-400">
                <button
                  ...
                  className="hover:text-white transition-colors p-2 rounded-full hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current"
                >
                  <Video size={20} />
                </button>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {loadingMessages ? (
                ...
                messages.map((msg) => (
                  ...
                  >
                    <div
                      className={`max-w-[70%] p-4 rounded-2xl shadow-lg relative ${msg.sender === "bot" ? "bg-purple-600/20 border border-purple-500/30 rounded-tr-sm text-purple-50" : "bg-white/5 border border-white/10 rounded-tl-sm text-gray-200"}`}
                    >
                      {msg.sender === "bot" && (
                      ...
                      <div
                        className={`text-[10px] flex items-center justify-end gap-1 mt-2 ${msg.sender === "bot" ? "text-purple-400" : "text-gray-500"}`}
                      >
                        {msg.time}
            ...
            <div className="p-4 border-t border-white/5 bg-[#0a0a0f]/80 backdrop-blur-md">
              <div className="flex items-end gap-2 bg-white/5 border border-white/10 rounded-2xl p-2 focus-within:border-purple-500/50 transition-colors">
                <button
                  ...
                  className="p-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current"
                >
                  <Smile size={20} />
                </button>
                <button
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-500 space-y-4">
            ...
            <p className="text-sm font-medium tracking-wide opacity-50 uppercase">
      ...
      <aside className="w-80 border-l border-white/5 bg-[#0a0a0f]/50 p-5 hidden xl:flex xl:flex-col">
        ...
        <div className="space-y-3">
          <PanelLink
            ...
            title="Contatos"
            desc="Ver fila de contatos e atendimentos"
          />
          <PanelLink
            href="/dashboard/whatsapp/builder"
            icon={<Workflow size={16} />}
            title="Automacoes"
            desc="Ajustar IA, agenda e roteamento"
          />
          <PanelLink
            href="/dashboard/rh/pipeline"
            icon={<Bot size={16} />}
            title="Pipeline RH"
            desc="Levar candidatos para triagem"
          />
        </div>
