...
function downloadCollectiveSheet(month: string, visibleEmployees: Employee[], byEmpMap: Record<string, TimeTrack[]>, allEmployees: Employee[], companyData: any, holidaysData: any[]) {
  ...
  const summaryData = [
    ...
    { label: 'Total Faltas no Período', value: String(visibleEmployees.reduce((acc, emp) => acc + buildGrid(month, emp, byEmpMap[emp.id] || [], (companyData as any)?.payrollStartDay, holidaysData).filter((g: any) => g.track && isFalta(g.track)).length, 0)) },
  ];
            ...
            <div key={e.id} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-semibold text-rose-600">{faltas} FALTA(S) • {e.department || '-'}</p>
                </div>
              </div>
