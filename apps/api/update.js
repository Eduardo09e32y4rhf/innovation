...
code = code.replace(
  ...
  'private async applyManual(companyId: string, employee: { id: string; dailyWorkload?: string | null; workScheduleRuleId?: string | null; workScale?: string | null; standardEntry?: string | null; standardExit?: string | null; }, dateValue: string, dto: Pick<ManualTimeTrackDto, \'entry\' | \'lunchStart\' | \'lunchReturn\' | \'exit\' | \'reason\' | \'observation\'>) {'
);

// 3. calculateTotals inside applyManual
code = code.replace(
  'const totals = this.calculateTotals(data, employee, date);',
  'const totals = await this.calculateTotals(companyId, data, employee, date);'
);

// 4. register call 1
code = code.replace(
  'const totals = this.calculateTotals({ ...current, [field]: now }, employee, today);',
  'const totals = await this.calculateTotals(companyId, { ...current, [field]: now }, employee, today);'
);

// 5. register call 2
code = code.replace(
  'const totals = this.calculateTotals({ ...current, [field]: timestamp }, employee, date);',
  'const totals = await this.calculateTotals(companyId, { ...current, [field]: timestamp }, employee, date);'
);

// 6. update call
code = code.replace(
  'const result = await this.repository.update(companyId, id, { ...data, ...this.calculateTotals(next, employee ?? undefined, next.date) });',
  'const totals = await this.calculateTotals(companyId, next, employee ?? undefined, next.date);\n    const result = await this.repository.update(companyId, id, { ...data, ...totals });'
);

// 7. calculateTotals definition
