...
export class DashboardRepository {
  ...
  private async buildInsights(employeeWhere: any, companyId: string) {
    ...
    // ⚡ Bolt: Consolidate multiple O(N) passes into a single loop
    const birthdaysToday: Array<{ id: string; name: string; birthDate: Date }> = [];
    const birthdaysThisMonth: Array<{ id: string; name: string; birthDate: Date }> = [];
    let missingUser = 0;
