...
export class DashboardRepository {
  ...
  private async buildInsights(employeeWhere: any, companyId: string) {
    ...
    type DashboardEmployee = typeof employees[0];
    const employeeIds = employees.map((employee: DashboardEmployee) => employee.id);
    ...
    const birthdaysToday = employees.filter((employee: DashboardEmployee) => employee.birthDate && employee.birthDate.getUTCMonth() + 1 === month && employee.birthDate.getUTCDate() === day).slice(0, 8);
    const birthdaysThisMonth = employees.filter((employee: DashboardEmployee) => employee.birthDate && employee.birthDate.getUTCMonth() + 1 === month).slice(0, 12);
    const missingUser = employees.filter((employee: DashboardEmployee) => employee.status === 'ACTIVE' && !employee.userId).length;
    const missingManager = employees.filter((employee: DashboardEmployee) => employee.status === 'ACTIVE' && !employee.managerId).length;
    const missingCpf = employees.filter((employee: DashboardEmployee) => !employee.cpf).length;
    const missingWorkScale = employees.filter((employee: DashboardEmployee) => employee.status === 'ACTIVE' && !employee.workScale).length;
    const missingWorkload = employees.filter((employee: DashboardEmployee) => employee.status === 'ACTIVE' && !employee.dailyWorkload).length;
    const company = await this.prisma.company.findUnique({ where: { id: companyId }, select: { name: true, document: true, logoUrl: true } });
    return {
      birthdaysToday: birthdaysToday.map((employee: DashboardEmployee) => ({ id: employee.id, name: employee.name, birthDate: employee.birthDate })),
      birthdaysThisMonth: birthdaysThisMonth.map((employee: DashboardEmployee) => ({ id: employee.id, name: employee.name, birthDate: employee.birthDate })),
