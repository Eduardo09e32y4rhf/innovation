...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    ...
    // Consolidate multiple aggregate calls into a single groupBy query
    // Reduces database roundtrips and connection overhead
    const revenue = Number(financialStats.find(s => s.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialStats.find(s => s.type === 'EXPENSE')?._sum.amount ?? 0);
