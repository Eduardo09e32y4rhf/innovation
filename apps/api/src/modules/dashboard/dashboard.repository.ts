...
export class DashboardRepository {
  ...
  private async buildInsights(employeeWhere: any, companyId: string) {
    ...
    const missingUser = employees.filter((employee: { status: string | null; userId: string | null }) => employee.status === 'ACTIVE' && !employee.userId).length;
    const missingManager = employees.filter((employee: { status: string | null; managerId: string | null }) => employee.status === 'ACTIVE' && !employee.managerId).length;
    const missingCpf = employees.filter((employee: { cpf: string | null }) => !employee.cpf).length;
    const missingWorkScale = employees.filter((employee: { status: string | null; workScale: string | null }) => employee.status === 'ACTIVE' && !employee.workScale).length;
    const missingWorkload = employees.filter((employee: { status: string | null; dailyWorkload: number | null }) => employee.status === 'ACTIVE' && !employee.dailyWorkload).length;
