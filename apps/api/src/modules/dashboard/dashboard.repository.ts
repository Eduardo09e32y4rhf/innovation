...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    const [
      ...
      // Expected impact: Reduces database roundtrips and connection overhead.
    ...
    const revenue = Number(financialTotals.find((t: any) => t.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialTotals.find((t: any) => t.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
