...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    const [
      ...
      // ⚡ Bolt Optimization: Consolidate multiple .aggregate() calls into a single .groupBy()
      // to reduce database roundtrips and connection overhead.
    ...
    const revenue = Number(transactionSums.find(r => r.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(transactionSums.find(r => r.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
