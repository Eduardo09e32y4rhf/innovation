...
export class DashboardRepository {
  ...
  private async buildInsights(employeeWhere: any, companyId: string) {
    ...
    // ⚡ Bolt: Optimize CPU performance by consolidating multiple O(N) filters into a single O(N) loop
    const birthdaysToday: (typeof employees)[0][] = [];
    const birthdaysThisMonth: (typeof employees)[0][] = [];
    let missingUser = 0;
    ...
    for (const employee of employees) {
      if (employee.birthDate) {
        ...
        if (empMonth === month) {
          if (birthdaysThisMonth.length < 12) birthdaysThisMonth.push(employee);
          if (employee.birthDate.getUTCDate() === day && birthdaysToday.length < 8) birthdaysToday.push(employee);
        }
      }
