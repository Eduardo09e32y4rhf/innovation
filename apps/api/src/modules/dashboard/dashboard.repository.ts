...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    const [
      ...
      // Optimization: Consolidate multiple aggregate calls into a single groupBy query
      // Expected impact: Reduces database roundtrips and connection overhead by fetching all aggregates at once.
    ...
    const revenue = Number(financialGroups.find((r) => r.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialGroups.find((r) => r.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
