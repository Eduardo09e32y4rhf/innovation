...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    const [
      ...
      // Bolt: Consolidated multiple .aggregate() calls into a single .groupBy() query to reduce DB roundtrips
    ...
    const revenue = Number(financialGrouped.find(g => g.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialGrouped.find(g => g.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
