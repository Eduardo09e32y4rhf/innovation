...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    const [
      ...
      // Consolidate multiple .aggregate() calls into a single .groupBy() query to reduce DB roundtrips
    ...
    const revenue = Number(financialSummary.find((r: any) => r.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialSummary.find((r: any) => r.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
