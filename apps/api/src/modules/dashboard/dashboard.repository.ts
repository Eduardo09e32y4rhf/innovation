...
export class DashboardRepository {
  ...
  async summary(companyId: string) {
    ...
    const revenue = Number(financialGrouped.find((r: any) => r.type === 'REVENUE')?._sum.amount ?? 0);
    const expenses = Number(financialGrouped.find((r: any) => r.type === 'EXPENSE')?._sum.amount ?? 0);
    return {
