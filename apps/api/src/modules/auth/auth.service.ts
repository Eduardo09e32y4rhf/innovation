...
export class AuthService {
  ...
  async validateResetCode(dto: { email: string; code: string; cpfStart: string; registration: string }) {
    ...
    if (typeof dto.code !== 'string') throw new UnauthorizedException('Código inválido ou expirado');
    ...
    if (typeof dto.cpfStart !== 'string' || !rawCpf.startsWith(dto.cpfStart.trim())) {
      throw new UnauthorizedException('Dados de validação incorretos');
    }

    const empReg = (employee.registration || '').trim().toLowerCase();
    const providedReg = typeof dto.registration === 'string' ? dto.registration.trim().toLowerCase() : '';
