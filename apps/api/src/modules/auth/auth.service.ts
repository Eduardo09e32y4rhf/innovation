...
export class AuthService {
  ...
  async validateResetCode(dto: { email: string; code: string; cpfStart: string; registration: string }) {
    if (!dto || typeof dto.email !== 'string' || typeof dto.code !== 'string' || typeof dto.cpfStart !== 'string' || typeof dto.registration !== 'string') {
      throw new BadRequestException('Dados de validação inválidos');
    }
