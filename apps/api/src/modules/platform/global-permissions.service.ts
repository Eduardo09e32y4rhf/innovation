  ...
  private async seedDefaults() {
    try {
      ...
      if (existingCount === 0) {
        ...
        const payload = Object.entries(DEFAULT_PERMISSIONS).map(([role, permissions]) => ({ role: role as UserRole, permissions }));
        await this.prisma.globalRolePermission.createMany({ data: payload });
      }
    } catch (e) {
