  ...
  private async seedDefaults() {
    try {
      ...
      if (existingCount === 0) {
        ...
        // ⚡ Bolt: Bulk insert to prevent N+1 queries during module init (reduces queries from 7 to 1)
        const dataToInsert = Object.entries(DEFAULT_PERMISSIONS).map(([role, permissions]) => ({ role: role as UserRole, permissions }));
