  ...
  async handleAsaasWebhook(
    ...
    if (event === 'PAYMENT_RECEIVED' || event === 'PAYMENT_CONFIRMED') {
      ...
      if (companyIdToActivate) {
        ...
        let targetInvoice = possibleInvoices.find((inv: { id: string; amount: any }) => Math.abs(Number(inv.amount) - Number(payment.value)) < 0.1);
      ...
      if (company) {
        ...
        let targetInvoice = openInvoices.find((inv: { id: string; amount: any }) => Math.abs(Number(inv.amount) - Number(payment.value)) < 0.1);
