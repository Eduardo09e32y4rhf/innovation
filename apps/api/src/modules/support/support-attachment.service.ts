...
export class SupportAttachmentService {
  ...
  constructor(
    ...
    private readonly fileValidationService: SupportFileValidationService,
  ...
  async uploadAttachment(actor: any, ticketId: string, file: any, messageId?: string) {
    const { extension } = this.fileValidationService.validate(file);
    ...
    await this.repository.createAttachment({
      ...
      declaredMimeType: this.fileValidationService.detectMimeType(extension, file.mimetype),
  ...
  async listAttachmentsByTicketId(ticketId: string) {
    return this.repository.findAttachmentsByTicketId(ticketId);
  }
