...
export class LocalSupportStorageService implements SupportStorageService {
  private readonly basePath =
    process.env.SUPPORT_ATTACHMENTS_PATH || "/data/attachments";
  private readonly logger = new Logger(LocalSupportStorageService.name);

  constructor() {
    this.ensureDirectory(this.basePath).catch((err) =>
      this.logger.error("Failed to create attachments directory", err),
    );
  }
