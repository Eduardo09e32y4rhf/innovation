...
function replaceModel(modelName, newContent) {
  ...
  if (schema.match(regex)) {
    schema = schema.replace(regex, newContent + '\\n');
  } else {
    schema += '\\n' + newContent + '\\n';
  }
}

['WorkScheduleRule', 'Holiday', 'TimeTrack'].forEach(modelName => {
  const regex = new RegExp('(model ' + modelName + ' \\{[\\s\\S]*?\\n\\})\\n*');
  const match = userSchema.match(regex);
  if (match) replaceModel(modelName, match[1]);
});

['TimeClosingSummary', 'OvertimeBank', 'TimeClosingAuditLog'].forEach(modelName => {
  const regex = new RegExp('(model ' + modelName + ' \\{[\\s\\S]*?\\n\\})\\n*');
  const match = userSchema.match(regex);
  if (match && !schema.includes('model ' + modelName)) {
    schema += '\\n' + match[1] + '\\n';
  }
});

if (schema.includes('model Employee {')) {
  if (!schema.includes('closingSummaries        TimeClosingSummary[]')) {
...
if (schema.includes('model User {')) {
  if (!schema.includes('overtimeApprovals       TimeTrack[]')) {
