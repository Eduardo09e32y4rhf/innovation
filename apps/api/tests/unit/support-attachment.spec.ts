...
describe('SupportAttachmentService', () => {
  ...
  beforeEach(() => {
    ...
    service = new SupportAttachmentService(repository as any, storageService as any);
  });
  ...
  describe('downloadAttachment (Isolamento Multi-Tenant)', () => {
    it('deve rejeitar se o anexo foi classificado como REJECTED (vírus)', async () => {
      repository.getAttachmentById.mockResolvedValue({ id: 'att-1', status: 'REJECTED' });
      
      await expect(service.downloadAttachment({ role: 'ADMIN' }, 'att-1'))
    ...
    it('deve rejeitar se o ator ADMIN for de uma empresa diferente do ticket', async () => {
      ...
      repository.findTicketById.mockResolvedValue({ companyId: 'company-B' });
      
      await expect(service.downloadAttachment({ role: 'ADMIN', companyId: 'company-A' }, 'att-1'))
    ...
    it('deve permitir se o ator FUNCIONARIO for o autor do ticket', async () => {
      ...
      repository.findTicketById.mockResolvedValue({ affectedUserId: 'user-1' });
      storageService.getFileStream.mockResolvedValue('fake-stream');

      const result = await service.downloadAttachment({ role: 'FUNCIONARIO', sub: 'user-1' }, 'att-1');
