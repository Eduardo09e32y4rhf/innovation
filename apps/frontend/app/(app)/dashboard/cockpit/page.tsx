        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    <div key={i} className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Gráfico Financeiro */}
                <div className="lg:col-span-2 bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col">
                    ...
                    <div className="flex-1 flex items-end gap-2 md:gap-3 px-2 min-h-[220px]">
                        {[40, 60, 45, 90, 65, 80, 55, 75, 60, 85, 70, 95].map((h, i) => (
                            <div key={i} className="flex-1 flex gap-1 items-end group relative h-full">
                                ...
                                {/* Tooltip */}
                                <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-50 text-slate-900 text-[9px] px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none shadow-xl">
                                    Sem {i + 1}
                                </div>
                            </div>
                        ))}
                ...
                <div className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                        ...
                        {[
                            ...
                            <div key={idx} className="p-4 hover:bg-slate-50 transition-colors flex gap-4 items-center group">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <span className={`inline-block mt-1 text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded bg-${item.color}-50 text-${item.color}-600 border-${item.color}-100`}>
                                        {item.type}
                                    </span>
                                </div>
                            </div>
...
const TabFinanceiro = ({ summary, transactions, dasMei }: { summary: any; transactions: any[]; dasMei: any }) => (
    ...
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-emerald-50/50 border-emerald-100 p-4 rounded-2xl flex items-center gap-4 transition-all hover:bg-emerald-50">
                <div className="p-3 bg-white border-slate-200 border-black/5 shadow-sm text-emerald-600 rounded-xl shadow-sm"><Landmark size={20} /></div>
                <div>
            ...
            <div className="bg-indigo-50/50 border-indigo-100 p-4 rounded-2xl flex items-center gap-4 transition-all hover:bg-indigo-50">
                <div className="p-3 bg-white border-slate-200 border-black/5 shadow-sm text-indigo-600 rounded-xl shadow-sm"><Building2 size={20} /></div>
                <div>
            ...
            <div className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-200 border-dashed p-4 rounded-2xl flex justify-center items-center cursor-pointer hover:bg-slate-50 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="space-y-6">
                {/* DAS / Impostos */}
                <div className="bg-slate-50 rounded-[2rem] p-8 text-slate-900 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <span className="bg-rose-500 text-slate-900 text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest mb-4 inline-block shadow-lg shadow-rose-900/40">
                            Vence em 2 dias
                        </span>
                        ...
                        <button className="w-full bg-white border-slate-200 border-black/5 shadow-sm text-slate-900 py-3.5 rounded-xl text-sm font-black transition-all hover:scale-[1.02] active:scale-[0.98] shadow-xl">
                            Gerar Boleto / Pix
                        </button>
                    </div>
                </div>
                ...
                {/* Contas a Pagar */}
                <div className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                        <h3 className="font-bold text-slate-800 flex items-center gap-2">
            ...
            {/* Extrato Principal */}
            <div className="lg:col-span-2 bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-3xl shadow-sm overflow-hidden flex flex-col">
                <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="relative w-full sm:w-64">
                        ...
                        <input type="text" placeholder="Filtrar por nome ou valor..." className="w-full pl-10 pr-4 py-2 text-xs border-slate-200 rounded-xl focus:outline-none focus:ring-2 ring-indigo-500 shadow-sm" />
                    </div>
                </div>
...
const TabPonto = () => {
    ...
    return (
        ...
        >
            <div className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-[3rem] p-8 md:p-12 shadow-2xl text-center relative overflow-hidden">
                ...
                <AnimatePresence mode="wait">
                    {pontoStatus === 'pendente' ? (
                        <motion.button
                            ...
                            className="w-full py-6 bg-slate-50 hover:bg-white text-slate-900 font-black text-xl rounded-2xl transition-all shadow-2xl flex justify-center items-center gap-3 group"
                        >
                        ...
                        <motion.div
                            ...
                            className="space-y-6"
                        >
                            <div className="inline-block px-6 py-3 bg-emerald-50 border-emerald-100 rounded-2xl">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button className="py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl transition-all border-slate-200">
                                    Intervalo
                                </button>
                                <button onClick={() => setPontoStatus('pendente')} className="py-4 bg-rose-600 hover:bg-rose-700 text-slate-900 font-bold rounded-2xl transition-all shadow-lg shadow-rose-100">
                                    Finalizar Dia
                                </button>
                            </div>
                        </motion.div>
                ...
                <div className="mt-12 pt-8 border-t border-slate-50 flex flex-col items-center gap-4">
                    ...
                    <p className="text-[10px] text-slate-700">IP: 172.19.0.1 • GPS: Sincronizado</p>
                </div>
            </div>
        </motion.div>
    );
...
const TabConfig = ({ user, company }: { user: UserProfile | null; company: any }) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-3xl shadow-sm p-8">
                {sub === 'profile' && (
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" defaultValue="+55 11 99999-9999" className="w-full bg-slate-50 border-slate-100 rounded-xl px-4 py-2.5 text-sm" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Senha</label>
                                <button className="w-full bg-slate-50 text-slate-900 rounded-xl px-4 py-2.5 text-sm font-bold">Alterar Senha</button>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-4 max-w-lg">
                            <div>
                                ...
                                <input type="text" defaultValue={company?.name || 'Inovação Tecnológica Ltda'} className="w-full bg-slate-50 border-slate-100 rounded-xl px-4 py-2.5 text-sm" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">CNPJ</label>
                                <input type="text" defaultValue={company?.cnpj || '54.694.364/0001-48'} className="w-full bg-slate-50 border-slate-100 rounded-xl px-4 py-2.5 text-sm" />
                            </div>
                ...
                {sub === 'billing' && (
                    <div className="space-y-6">
                        <div className="p-6 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl text-slate-900 shadow-xl">
                            ...
                            <button className="bg-white border-slate-200 border-black/5 shadow-sm text-indigo-700 px-6 py-2.5 rounded-xl font-bold text-sm shadow-xl">Gerenciar Assinatura</button>
                        </div>
                    </div>
                )}
            </div>
...
const TabAssistente = () => (
    <div className="h-full flex flex-col items-center justify-center py-20">
        <div className="relative mb-10">
            ...
            <div className="relative w-24 h-24 bg-white border-slate-200 border-black/5 shadow-sm border-indigo-100 rounded-[2.5rem] flex items-center justify-center text-indigo-600 shadow-2xl">
                <Bot size={48} />
            </div>
        </div>
...
export default function ModernDashboard() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans flex flex-col lg:flex-row text-slate-900 selection:bg-indigo-100 overflow-hidden">
            ...
            {/* MOBILE HEADER */}
            <div className="lg:hidden flex items-center justify-between bg-white border-slate-200 border-black/5 shadow-sm p-5 border-b border-slate-100 z-[100]">
            ...
            {/* SIDEBAR */}
            <aside className={`
        fixed inset-y-0 left-0 w-72 bg-white border-slate-200 border-black/5 shadow-sm border-r border-slate-100 z-[90] flex flex-col transition-transform duration-300
      ...
      `}>
                ...
                <div className="px-6 mb-6">
                    <div className="bg-slate-50 border-slate-50 rounded-2xl p-4 flex items-center gap-4 shadow-inner">
                        <div className="w-10 h-10 bg-white border-slate-200 border-black/5 shadow-sm rounded-xl flex items-center justify-center font-black text-indigo-600 shadow-sm">
                            {user?.name?.[0] || 'U'}
                        </div>
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden bg-white border-slate-200 border-black/5 shadow-sm lg:bg-slate-50/50">
                ...
                <header className="hidden lg:flex h-20 px-10 items-center justify-between border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 rounded-full border-emerald-100">
                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                            <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Sincronizado</span>
                        </div>

                        <button className="relative p-2.5 bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-xl text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
                            <Bell size={20} />
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="w-10 h-10 bg-slate-50 text-slate-900 rounded-xl flex items-center justify-center font-black shadow-lg">
                                {user?.name?.[0] || 'A'}
                            </div>
                        </div>
                    </div>
