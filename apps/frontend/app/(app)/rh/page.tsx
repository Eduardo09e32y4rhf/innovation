...
function StatCard({
  ...
  return (
    <div className="relative overflow-hidden bg-white border-slate-200 border-black/5 shadow-sm/70 backdrop-blur-xl border-slate-200/60 p-6 rounded-3xl shadow-sm hover:shadow-md transition-all group">
      <div
        className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${colorClass} opacity-[0.04] group-hover:opacity-[0.08] transition-opacity rounded-bl-full`}
      />
      <div className="relative z-10 flex flex-col h-full justify-between gap-4">
        <div className="flex justify-between items-start">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 ${colorClass.split(' ')[0].replace('from-', 'text-')}`}
          >
            <Icon size={22} strokeWidth={2} />
          </div>
          {trend && (
            <span
              className={`flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${trend.startsWith('+') || trend.startsWith('↑') ? 'bg-emerald-50 text-emerald-600 border-emerald-100/50' : 'bg-amber-50 text-amber-600 border-amber-100/50'}`}
            >
        ...
        <div>
          <h3 className="text-[13px] font-medium text-slate-500 mb-1">
            {title}
          </h3>
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div
          className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white ${employee.status === 'active' ? 'bg-emerald-500' : 'bg-amber-500'} shadow-[0_0_0_1px_rgba(0,0,0,0.05)]`}
        />
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="text-sm font-semibold text-slate-800 truncate group-hover:text-blue-600 transition-colors tracking-tight">
          {employee.name}
        </h3>
        <p className="text-[12px] text-slate-500 truncate mt-0.5">
          {employee.role} <span className="text-slate-700 mx-1">•</span>{' '}
          {employee.department}
        </p>
      </div>
      <div className="flex flex-col items-end gap-2">
        <div
          className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold tracking-wide border ${riskStyles[employee.risk]}`}
        >
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-50/20 backdrop-blur-sm z-[100]"
            onClick={onClose}
          />
          <motion.div
            initial={{ x: '100%' }}
          ...
          >
            <div className="flex justify-between items-start mb-8">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-600 flex items-center justify-center text-xl font-bold text-slate-900 shadow-lg shadow-blue-200">
                  {employee?.name[0]}
                </div>
                <div>
              ...
              <button
                ...
                aria-label="Fechar"
                title="Fechar"
                className="p-2 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100/80">
                <p className="text-[11px] text-slate-500 font-medium tracking-wide mb-1 flex items-center gap-1.5">
                  <Sparkles size={12} /> Status Cognitivo
                </p>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100/80">
                <p className="text-[11px] text-slate-500 font-medium tracking-wide mb-1 flex items-center gap-1.5">
                  <Timer size={12} /> Tempo de Empresa
                </p>
            ...
            <div className="space-y-6 relative before:absolute before:left-[15px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
              {loading ? (
                [1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-20 bg-slate-50 rounded-2xl animate-pulse ml-10"
                  />
                ))
              ) : events.length === 0 ? (
      ...
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-[200] p-4">
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-slate-50/30 backdrop-blur-sm"
            onClick={onClose}
          />
          <motion.div
          ...
          >
            <div className="w-16 h-16 bg-gradient-to-tr from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center mx-auto mb-6 border-indigo-50 text-blue-600 shadow-inner">
              <UserPlus size={28} />
            </div>
            ...
            <p className="text-[13px] text-slate-500 mb-8 px-2 leading-relaxed">
              Inicie o onboarding "Zero Papel". O talento receberá um link
              seguro para envio de documentos.
            </p>
            ...
            {done ? (
              ...
              <div className="space-y-5">
                <div className="relative text-left">
                  <label className="text-[11px] font-semibold text-slate-500 mb-1.5 block ml-1 uppercase tracking-wide">
                    E-mail do Talento
                  </label>
                  <div className="relative">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 bg-blue-600 hover:bg-indigo-700 text-slate-900 font-semibold py-3 rounded-xl shadow-md shadow-blue-200 transition-all text-sm disabled:opacity-50 disabled:hover:bg-blue-600 flex justify-center items-center gap-2"
                  >
                    {sending ? (
                      <>
      ...
      <div className="p-6 sm:p-8 space-y-8 animate-in fade-in duration-500 min-h-screen">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white border-slate-200 border-black/5 shadow-sm/70 backdrop-blur-xl p-6 rounded-[2rem] border-slate-200/60 shadow-sm">
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 bg-gradient-to-tr from-indigo-600 to-violet-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 text-slate-900">
              <Heart size={26} />
            </div>
            <div>
              ...
              <p className="text-[13px] text-slate-500 font-medium">
                Gestão inteligente e desenvolvimento continuado de talentos.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0">
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-indigo-700 text-slate-900 px-5 py-2.5 rounded-xl font-semibold text-[13px] flex items-center gap-2 shadow-sm shadow-blue-200 transition-all"
            >
              <UserPlus size={16} />{' '}
            ...
            <div className="bg-white border-slate-200 border-black/5 shadow-sm/70 backdrop-blur-xl border-slate-200/60 rounded-[2rem] p-6 sm:p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2">
                  <Users size={20} className="text-blue-600" /> Quadro de
                  Colaboradores
                </h2>
                <div className="flex gap-1">
                  <button
                    aria-label="Filtrar"
                    title="Filtrar"
                    className="p-2 text-slate-400 hover:text-blue-600 hover:bg-indigo-50 rounded-xl transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                  >
                    <Filter size={18} />
                  </button>
                  <button
              ...
              {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className="h-20 bg-slate-50 border-slate-100 rounded-[1.25rem] animate-pulse"
                    />
                  ))}
                </div>
              ) : (
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white border-slate-200 border-black/5 shadow-sm/70 backdrop-blur-xl border-rose-100/60 p-6 sm:p-8 rounded-[2rem] shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2 mb-6">
                <AlertCircle size={20} className="text-rose-500" /> Alertas de
                Risco (IA)
              </h2>
                ...
                {employees.filter((e) => e.risk !== 'stable').length === 0 ? (
                  <div className="text-center py-8 border-dashed border-emerald-200/50 rounded-2xl bg-emerald-50/30">
                    ...
                    <p className="text-[13px] font-medium text-slate-500">
                      Nenhum risco detectado pela análise cognitiva.
                    </p>
                  </div>
                ) : (
                  employees
                    .filter((e) => e.risk !== 'stable')
                    .map((emp) => (
                      <div
                        key={emp.id}
                        className="flex items-center gap-3 p-3.5 rounded-[1.25rem] bg-white border-slate-200 border-black/5 shadow-sm border-rose-100/50 shadow-sm hover:shadow-md hover:border-rose-200 transition-all cursor-pointer group"
                        onClick={() => {
                          setSelectedEmp(emp);
                          setIsDrawerOpen(true);
                        }}
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2rem] p-6 sm:p-8 text-slate-900 shadow-xl shadow-blue-200/50 group relative overflow-hidden">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h3 className="text-base font-bold tracking-tight">
                        Onboarding Zero Papel
                      </h3>
                    </div>
                    <p className="text-[11px] font-semibold text-indigo-200 tracking-wide">
                      COMPLIANCE 100% DIGITAL
                    </p>
                  </div>
                </div>
                <div className="space-y-4 bg-white border-slate-200 border-black/5 shadow-sm/10 backdrop-blur-sm border-white/10 rounded-2xl p-4">
                  ...
                  <div className="h-1.5 bg-white border-slate-200 border-black/5 shadow-sm/20 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className="h-full bg-white border-slate-200 border-black/5 shadow-sm w-full rounded-full shadow-[0_0_10px_white]"
                    />
                  </div>
                  <p className="text-[11px] leading-relaxed text-indigo-100 pt-1">
                    "A plataforma validou 12 novos documentos via IA hoje,
                    economizando aproximadamente 4h de triagem humana."
                  </p>
                </div>
              </div>
            </div>
