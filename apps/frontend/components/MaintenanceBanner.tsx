...
export function MaintenanceBanner() {
        ...
        if (response.ok) {
          ...
          if (data.maintenance_mode) {
            setBanner({
              ...
              type: 'maintenance',
              message:
                data.maintenance_message || 'Sistema em manutenção programada.',
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-[100] ${style.bg} ${style.border} border-b backdrop-blur-xl`}
    >
      <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-center gap-3">
        ...
        <p className="text-sm text-slate-900 font-medium">
          {banner.message}
          {banner.eta && (
            <span className="text-slate-900/70 ml-2">
              (Previsão: {banner.eta})
            </span>
          )}
        </p>
        <button
          ...
          className="text-slate-900/60 hover:text-slate-900 transition ml-2 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current rounded"
