...
Write-Host "Extraindo arquivos e subindo containers no VPS..."
# Executa os comandos remotamente no VPS via SSH
ssh ${vpsUser}@${vpsIp} "mkdir -p $vpsPath && tar -xzvf /root/deploy.tar.gz -C $vpsPath && rm /root/deploy.tar.gz && cd $vpsPath && docker compose -f infrastructure/ops/docker-compose.yml down && docker compose -f infrastructure/ops/docker-compose.yml build && docker compose -f infrastructure/ops/docker-compose.yml up -d"
