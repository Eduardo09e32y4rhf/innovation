...
def request_code(db: Session, user_id: int, email: str | None, phone: str | None):
    ...
    expires_at = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(seconds=CODE_TTL_SECONDS)
...
def verify_code(db: Session, user_id: int, code: str):
    two_factor_code = db.query(TwoFactorCode).filter(TwoFactorCode.user_id == user_id, TwoFactorCode.used == False).first()
