...
- Name: platform-finance.spec.ts >> Plataforma: Asaas e cobrancas >> sincroniza o status de uma cobranca existente
- Location: tests/platform-finance.spec.ts:79:7
...
Call log:
  - Expect "toHaveURL" with timeout 15000ms
    34 × unexpected value "http://127.0.0.1:3000/login"

```

```yaml
- img "Innovation RH Connect"
- paragraph: Gestão de RH inteligente para o seu negócio
- heading "Entrar na Plataforma" [level=1]
- paragraph: Digite suas credenciais corporativas abaixo.
- text: E-mail corporativo
- textbox "E-mail corporativo":
...
```ts
  ...
  23 |   await page.locator('#email').fill(DEV_EMAIL);
  24 |   await page.locator('#password').fill(DEV_PASSWORD);
