...
function StatCard({ stat, index }: { stat: any; index: number }) {
    ...
    return (
        <div
            className="group relative bg-purple-500/[0.03] border border-purple-200 rounded-2xl p-5 hover:border-[#8b5cf6]/40 hover:bg-purple-500/[0.05] transition-all duration-300 overflow-hidden"
        ...
        >
            ...
            <p className="text-xs text-slate-900/40 uppercase tracking-wider mb-1">{stat.title}</p>
            <p className="text-2xl font-black text-slate-900">
                {stat.prefix}{count.toLocaleString('pt-BR')}{stat.suffix}
            </p>

            <div className="mt-3 flex items-center justify-between">
                <div className="flex-1 h-1 bg-purple-500/5 rounded-full mr-3">
                    <div
    ...
    return (
        <div className="flex gap-1 items-end overflow-x-auto pb-2">
            <div className="flex flex-col gap-1 mr-1 shrink-0">
                {dayLabels.map((d, i) => (
                    <span key={i} className="text-[9px] text-slate-900/20 leading-none" style={{ height: '10px', lineHeight: '10px' }}>{d}</span>
                ))}
            </div>
            {grid.map((week, w) => (
            ...
            <div className="min-h-screen text-slate-900">
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {/* Ranking de Módulos */}
                        <div className="lg:col-span-1 bg-purple-500/[0.02] border border-purple-200 rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    <h2 className="text-sm font-bold text-slate-900">Módulos Mais Usados</h2>
                                    <p className="text-[10px] text-slate-900/30">Seu ranking de uso</p>
                                </div>
                                ...
                                {moduleRanking.map((m, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex items-center gap-3 group">
                                            ...
                                            <div className="w-8 h-8 rounded-lg bg-purple-500/5 flex items-center justify-center shrink-0">
                                                <Icon className={`w-4 h-4 ${m.color}`} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="text-xs font-medium text-slate-900/80">{m.name}</span>
                                                    <span className="text-[10px] text-slate-900/30">{m.uses.toLocaleString()}</span>
                                                </div>
                        ...
                        {/* Gráfico de Receita */}
                        <div className="lg:col-span-2 bg-purple-500/[0.02] border border-purple-200 rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    <h2 className="text-sm font-bold text-slate-900">Evolução da Receita</h2>
                                    <p className="text-[10px] text-slate-900/30">Últimos 6 meses</p>
                                </div>
                            ...
                            <div className="flex items-end justify-between gap-3 h-40">
                                {([
                                    ...
                                    <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
                                        <span className="text-[10px] text-slate-900/50 group-hover:text-slate-900 transition-colors">{d.v}%</span>
                                        <div className="w-full bg-purple-500/5 rounded-lg relative overflow-hidden" style={{ height: '120px' }}>
                                            <div
                                        ...
                                        <span className="text-[10px] text-slate-900/30">{d.month}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 bg-purple-500/[0.02] border border-purple-200 rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-900/30">Feed em tempo real</p>
                                </div>
                                <Activity className="w-5 h-5 text-[#8b5cf6]" />
                            </div>

                            {/* Heatmap */}
                            <div className="mb-5 p-3 bg-purple-500/[0.02] rounded-xl border border-purple-200">
                                <p className="text-[9px] text-slate-900/20 uppercase tracking-widest mb-2">Mapa de Atividade — últimas 12 semanas</p>
                                ...
                                {loading ? (
                                    [...Array(3)].map((_, i) => (
                                        <div key={i} className="flex items-center gap-3 animate-pulse">
                                            <div className="w-9 h-9 rounded-full bg-purple-500/5" />
                                            <div className="flex-1 space-y-1">
                                                <div className="h-3 bg-purple-500/5 rounded w-3/4" />
                                    ...
                                    [
                                        ...
                                        <div key={i} className="flex items-center gap-3 group hover:bg-purple-500/[0.02] px-2 py-1.5 rounded-xl transition-all">
                                            <div className="w-9 h-9 rounded-full bg-purple-500/5 border border-purple-200 flex items-center justify-center text-base shrink-0">
                                                {item.badge}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xs font-semibold text-slate-900">{item.name}</p>
                                                <p className="text-[11px] text-slate-900/40 truncate">{item.msg}</p>
                                            </div>
                                            <span className="text-[10px] text-slate-900/20 shrink-0">{item.time}</span>
                                        </div>
                                    ))
                                ) : activities.slice(0, 4).map((item, i) => (
                                    <div key={i} className="flex items-center gap-3 hover:bg-purple-500/[0.02] px-2 py-1.5 rounded-xl transition-all">
                                        <div className="w-9 h-9 rounded-full bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 flex items-center justify-center shrink-0">
                                            <Activity className="w-4 h-4 text-[#8b5cf6]" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-semibold text-slate-900">{item.candidate_name || 'Sistema'}</p>
                                            <p className="text-[11px] text-slate-900/40 truncate">{item.message}</p>
                                        </div>
                                        <span className="text-[10px] text-slate-900/20 shrink-0">
                    ...
                    {/* ── QUICK ACCESS ─────────────────────────────────────────────── */}
                    <div>
                        <h2 className="text-sm font-bold text-slate-900/60 uppercase tracking-widest mb-4">Acesso Rápido</h2>
                            ...
                            {[
                                ...
                                return (
                                    <Link key={i} href={item.href}
                                        className="flex flex-col items-center gap-2 p-4 bg-purple-500/[0.02] border border-purple-200 rounded-2xl hover:border-[#8b5cf6]/30 hover:bg-purple-500/[0.05] transition-all group">
                                        ...
                                        <span className="text-xs font-medium text-slate-900/60 group-hover:text-slate-900 transition-colors">{item.label}</span>
                                    </Link>
                                );
                            })}
                        </div>
                    ...
                    {user?.role === 'admin' && (
                        <div>
                            <h2 className="text-sm font-bold text-slate-900/60 uppercase tracking-widest mb-4">Administração de IA</h2>
                            <AIKeyManager />
                        </div>
                    )}
