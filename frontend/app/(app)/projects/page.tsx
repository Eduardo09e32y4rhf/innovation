                ...
                {projects.length > 0 && (
                    <div className="bg-white border-slate-200 border-black/5 shadow-sm border-slate-100 rounded-[2rem] p-4 flex flex-col md:flex-row gap-4 shadow-sm items-center">
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            ...
                            <button onClick={createTask} className="w-14 h-14 bg-indigo-600 text-white rounded-[1.5rem] flex items-center justify-center shadow-lg shadow-indigo-100 hover:scale-[1.05] active:scale-95 transition-all" aria-label="Criar tarefa">
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                                                <div className="flex gap-2">
                                                    {col !== 'todo' && (
                                                        <button onClick={() => moveTask(task.id, COLUMNS[COLUMNS.indexOf(col) - 1])} className="w-8 h-8 rounded-xl bg-slate-50 text-slate-700 hover:bg-slate-100 hover:text-slate-600 transition-all flex items-center justify-center" aria-label="Mover tarefa para a coluna anterior">←</button>
                                                    )}
                                                    {col !== 'done' && (
                                                        <button onClick={() => moveTask(task.id, COLUMNS[COLUMNS.indexOf(col) + 1])} className="w-8 h-8 rounded-xl bg-slate-50 text-slate-700 hover:bg-slate-100 hover:text-slate-600 transition-all flex items-center justify-center" aria-label="Mover tarefa para a próxima coluna">→</button>
                                                    )}
                                                </div>
