...
import { Key, Plus, Trash2, RefreshCw, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function AIKeyManager() {
    ...
    const handleRemoveKey = async (keyId: string) => {
        ...
        try {
            const res = await fetch(`/api/ai-admin/keys/${encodeURIComponent(keyId)}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchKeys();
    ...
    const handleReset = async () => {
        if (!confirm('Isso resetará o status de TODAS as chaves para ATIVO. Continuar?')) return;

        try {
            const res = await fetch('/api/ai-admin/keys/reset', { method: 'POST' });
            if (res.ok) {
                fetchKeys();
                setMessage({ text: 'Todas as chaves foram resetadas!', type: 'success' });
                setTimeout(() => setMessage(null), 3000);
            }
        } catch (err) {
                    ...
                    {keys.map((k) => (
                        ...
                        >
                            ...
                            <button
                                ...
                                className="p-2 rounded-lg bg-red-500/0 hover:bg-red-500/10 text-red-500/40 hover:text-red-500 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/50"
            ...
            <AnimatePresence>
                {message && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 50 }}
                        className={`fixed bottom-6 right-6 flex items-center gap-3 px-6 py-4 rounded-2xl border backdrop-blur-2xl shadow-2xl z-50 ${message.type === 'success' ? 'bg-green-500/20 border-green-500/50 text-green-200' : 'bg-red-500/20 border-red-500/50 text-red-200'
                            }`}
                    >
