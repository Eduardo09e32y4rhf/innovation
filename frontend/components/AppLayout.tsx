...
import {
    ...
    Bell, Settings, ArrowLeft, Flame, Trophy
} from 'lucide-react';
...
// ─── MENU CONFIG (módulos activos no front) ──────────────────────────────────
const MAIN_MENU = [
...
function Sidebar({ user }: { user: UserProfile | null }) {
    ...
    return (
        <aside className="fixed left-0 top-0 bottom-0 w-[240px] bg-black/70 backdrop-blur-2xl border-r border-white/5 z-50 flex flex-col">
            {/* Logo */}
            <div className="px-5 py-5 border-b border-white/5 shrink-0">
                ...
                <p className="text-[9px] text-white/20 mt-0.5 tracking-widest uppercase">Enterprise Platform</p>
            </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-0.5">
                                ...
                                <span className="text-[9px] text-orange-400 font-medium">7 dias seguidos</span>
                            </div>
                        </div>
...
// ─── TOP BAR ────────────────────────────────────────────────────────────────
function TopBar({ user, title }: { user: UserProfile | null; title?: string }) {
    const router = useRouter();
    const pathname = usePathname();
    const isHome = pathname === '/dashboard';

    return (
        <header className="sticky top-0 z-40 bg-black/60 backdrop-blur-xl border-b border-white/5 px-6 py-3 flex items-center gap-4">
            {/* Back button */}
            {!isHome && (
                <button
                    onClick={() => router.back()}
            ...
            <div className="flex items-center gap-3 shrink-0">
                ...
                {user && (
                    <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/8 cursor-default">
                        ...
                        <span className="text-[9px] bg-[#8b5cf6]/20 text-[#a78bfa] px-1.5 py-0.5 rounded font-bold">Nv.7</span>
                    </div>
                )}
            </div>
        </header>
