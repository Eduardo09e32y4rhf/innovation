...
def run():
    with sync_playwright() as p:
        ...
        page.route(
            "**/api/rh/v2/pdi", lambda route: route.fulfill(status=200, body="[]")
        )
        page.route(
            "**/api/rh/v2/time-bank/balance",
            lambda route: route.fulfill(
                status=200,
                body='{"total_credit_hours": 0, "total_debit_hours": 0, "balance_hours": 0}',
            ),
        )
        page.route(
            "**/api/rh/v2/payslips/me",
            lambda route: route.fulfill(status=200, body="[]"),
        )
        page.route(
            "**/api/rh/attendance/history*",
            lambda route: route.fulfill(status=200, body="[]"),
        )
        page.route(
            "**/api/analytics",
