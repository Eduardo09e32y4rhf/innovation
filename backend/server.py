...
TICKETS_MAP = {t["id"]: t for t in MOCK_TICKETS}
...
MOCK_MESSAGES = [
    ...
    },
    {
        "id": 2,
        "ticketId": 1,
        "message": "Olá! Claro, em que posso ajudar?",
        "fromMe": True,
...
def login(data: LoginRequest):
    ...
    for user in MOCK_USERS:
        if user["email"] == data.email and user["password"] == data.password:
            ...
            return LoginResponse(user=user, token=token, company=MOCK_COMPANIES[0])
...
def get_companies(
    authorization: Optional[str] = Header(None), plan_id: Optional[str] = None
):
...
def get_ticket(ticket_id: int, authorization: Optional[str] = Header(None)):
    """Pegar ticket"""
    verify_token(authorization)
    # ⚡ Bolt: [performance improvement] Uses O(1) dictionary lookup via TICKETS_MAP instead of O(N) list traversal over MOCK_TICKETS.
    if ticket_id in TICKETS_MAP:
        return TICKETS_MAP[ticket_id]
...
def create_ticket(
    ...
    # ⚡ Bolt: [performance improvement] Keep O(1) lookup map in sync when creating new items.
...
def update_ticket(
    ...
    """Atualizar ticket"""
    verify_token(authorization)
    # ⚡ Bolt: [performance improvement] Uses O(1) dictionary lookup via TICKETS_MAP instead of O(N) list traversal.
    if ticket_id in TICKETS_MAP:
        ticket = TICKETS_MAP[ticket_id]
        if status:
...
def get_messages(ticket_id: int, authorization: Optional[str] = Header(None)):
...
def set_language(lang: str, authorization: Optional[str] = Header(None)):
...
def set_language_alias(lang: str, authorization: Optional[str] = Header(None)):
...
def get_user_settings(user_id: int, authorization: Optional[str] = Header(None)):
    """Pegar configurações do usuário"""
    verify_token(authorization)
    return {"language": "pt", "theme": "dark", "queues": []}
...
def get_whatsapp_instances(
    session: Optional[str] = None, authorization: Optional[str] = Header(None)
):
...
def connect_whatsapp(authorization: Optional[str] = Header(None)):
    ...
    WHATSAPP_STATE["qr"] = (
        f"innovation-ia-demo:{WHATSAPP_STATE['session']}:{int(datetime.now().timestamp())}"
    )
    return {
        "session": WHATSAPP_STATE["session"],
        "status": WHATSAPP_STATE["status"],
