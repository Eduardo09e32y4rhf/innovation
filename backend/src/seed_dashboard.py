...
def seed_dashboard_data():
    ...
    try:
        ...
        if not user:
            print("Nenhum usuário 'company' encontrado. Por favor, registre um primeiro.")
            return
        ...
        now = datetime.now()
        for i in range(6):
            month_date = (now - timedelta(days=i*30)).replace(day=random.randint(1, 28))
        ...
        # 3. Criar Vagas (ATS)
        titles = ["Senior Python Developer", "UI/UX Designer", "Product Manager", "DevOps Engineer"]
        jobs = []
        for title in titles:
            job = Job(
                title=title,
        ...
        # 5. Criar Projetos
        prj_names = ["Integração IA Cloud", "Portal de Talentos v2", "Dashboard Executivo"]
