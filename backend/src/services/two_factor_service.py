...
def request_code(
    db: Session, user_id: int, email: str | None, phone: str | None
) -> str:
    """
    ...
    two_factor_code = TwoFactorCode(
        user_id=user_id, code=code, expires_at=expires_at, attempts=0
    )
...
def verify_code(db: Session, user_id: int, code: str) -> bool:
    ...
    """
    two_factor_code = (
        db.query(TwoFactorCode).filter(TwoFactorCode.user_id == user_id).first()
    )
    ...
    if two_factor_code.code != code:
        ...
        db.commit()
        logger.warning(
            f"Código 2FA incorreto para user_id={user_id} (tentativa {two_factor_code.attempts}/{_MAX_ATTEMPTS})"
        )
        return False
