...
class FinanceService:
    ...
    def ai_cash_flow_prediction(db: Session, company_id: int):
        ...
        return {
            ...
            "recommended_action": "Priorizar pagamento de impostos e fornecedores críticos." if status == "alerta" else "Investir excedente em CDB ou Tesouro."
        }
    ...
    def detect_anomalies(db: Session, company_id: int):
        ...
        anomalies = []
        stats_map = {e.category: (float(e.avg_amount or 0), float(e.std_amount or 0)) for e in expenses}
        ...
        if not anomalies:
            return [{"description": "Nenhuma anomalia detectada recentemente.", "impact": "Baixo", "suggestion": "Monitoramento continua ativo."}]
