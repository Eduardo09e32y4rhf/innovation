...
def get_cost_centers(
    ...
    # ⚡ Bolt: Fetch aggregated transaction totals directly from the database
    # Why: Previously fetched all transaction records into memory just to calculate sum totals (O(N) memory allocation).
    # Impact: Reduces O(N) database transfer and memory load to strictly bounded size, improving response time.
    ...
    total_spend = sum(row.total or 0.0 for row in aggregated_centers)

    centers_list = []
    for row in aggregated_centers:
        cat_total = row.total or 0.0
        centers_list.append(
            {
                ...
                "percentage": (
                    round(cat_total / total_spend * 100, 1) if total_spend else 0
                ),
            }
        )
