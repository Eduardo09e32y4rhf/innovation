...
def get_cost_centers(
    ...
    # Why: Previously, this fetched all transactions into memory and used Python loops to group by category.
    ...
    centers = []
    total_spend = sum(float(row.total) for row in results if row.total)
        ...
        centers.append(
            {
                ...
                "count": row.count,
                "percentage": (
                    round((total / total_spend) * 100, 1) if total_spend else 0
                ),
            }
        )
