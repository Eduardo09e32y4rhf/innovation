...
def get_cost_centers(
    ...
    # ⚡ Bolt: Fetch aggregated cost centers via a single database query
    # Why: Previously executed `.all()` to fetch every transaction object into Python memory
    # just to compute sums per category. For growing accounts, this caused O(N) memory allocation.
    # Impact: Reduces O(N) memory allocations to O(C) (C=number of categories), bounds python processing,
    # and utilizes the database for fast math via `SUM()` and `GROUP BY`.
    ...
    total_spend = sum(r.total for r in results if r.total)
