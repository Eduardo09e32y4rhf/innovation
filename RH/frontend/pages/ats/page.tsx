...
export default function ATSPage() {
    ...
    const [form, setForm] = useState<JobFormState>({ title: '', description: '', location: '', type: 'CLT' });
    ...
    const filteredJobs = jobs.filter((job) => {
        const haystack = `${job.title} ${job.location || ''} ${job.type || ''} ${job.description || ''}`.toLowerCase();
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <div className="relative flex-1 md:w-64">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-50 border-slate-100 rounded-xl py-2.5 pl-11 pr-4 text-xs font-medium focus:ring-4 focus:ring-indigo-50 outline-none transition-all"
                                />
                            </div>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-20 bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-100">
                            ...
                            <p className="text-sm font-black text-slate-400 uppercase tracking-widest">
                                {searchTerm ? 'Nenhuma vaga corresponde ao filtro.' : 'Nenhuma vaga publicada no momento.'}
                            </p>
