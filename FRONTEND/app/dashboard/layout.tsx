    ...
    <div className="flex h-screen bg-[#05050a] text-white overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/5 flex flex-col glass z-50">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <NavItem icon={<MessageSquare size={20} />} label="Chat (WhatsApp)" href="/dashboard/whatsapp/builder" />
          <NavItem icon={<Users size={20} />} label="Recursos Humanos" href="/dashboard/rh/pipeline" />
          <NavItem icon={<CreditCard size={20} />} label="Financeiro" href="/dashboard/finance/pricing" />
          <NavItem icon={<Cpu size={20} />} label="AI Engines" href="/dashboard/rh/pipeline" />
        </nav>

        <div className="p-4 border-t border-white/5">
          <NavItem icon={<Settings size={20} />} label="Configurações" />
          <NavItem icon={<LogOut size={20} />} label="Sair" className="text-red-400" />
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative overflow-y-auto">
        ...
        <header className="h-16 border-b border-white/5 bg-black/20 backdrop-blur-xl flex items-center justify-between px-8 sticky top-0 z-40">
          <div className="flex items-center bg-white/5 border border-white/10 rounded-full px-4 py-1.5 w-96">
            ...
            <input type="text" placeholder="Buscar em toda a plataforma..." className="bg-transparent border-none focus:ring-0 text-sm ml-2 w-full text-gray-300" />
          </div>
...
const NavItem = ({ icon, label, href = "#", active = false, className = "" }) => (
  <a href={href} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all group ${
